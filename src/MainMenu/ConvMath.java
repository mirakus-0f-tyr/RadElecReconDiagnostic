...
public class ConvMath {
    ...
    // pressure units - these aren't used anywhere yet
    public static String UStoSIPressure(double inHg) {
        return formatSI_pres.format(inHg * 25.4);
    }

    public static String SItoUSPressure(double mmHg) {
        return formatUS_pres.format(mmHg / 25.4);
    }

    public static String UStoMbar(double inHg) {
        return formatSI_pres.format(inHg * 33.8639);
    }

    public static String mBarToUS(double mBar) {
        return formatUS_pres.format(mBar / 33.8639);
    }
}
